...
f(u) &=  e^{-k\frac{u^2}{2}} e^c, \quad \text{let A = $e^c$} \\
...
\section{Conclusion}
This paper develops a foundational stochastic framework for high-frequency market making. Starting from independent Bernoulli trade events, we derive Poisson arrivals and exponential inter-trade waiting times, representing trades in event time, not physical time. Under the iid, finite second moment, and martingale pricing constraining assumption, we allow observed prices to be represented by valuation uncertainty through aggregation in infinitely repeated trading and inferred in finite trading, which calls for continuously updating beliefs.

Within the constructed structure, Bayesian conjugate updating emerges as the unique mechanism for continuously evolving valuation under finite information while remaining consistent with aggregation asymptotic limits.  Furthermore, we distinguish the risk from price fluctuations from the risk of model failure. In the case of pricing risk, adaptive spread control is adopted to protect the system from adverse selection or inventory and liquidity risk. On the other hand, model failure risk initiates a market withdrawal mechanism, which protects the system from potentially high risk exposure that arises from model failure and continued quoting.
