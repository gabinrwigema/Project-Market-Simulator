...
\section{Conclusion}
This paper develops a unified stochastic framework for high-frequency market making by deriving price dynamics, valuation inference, and trading controls directly from discrete trade arrivals. Starting from Bernoulli trade events, we obtain Poisson arrivals and exponential inter-trade waiting times, yielding an event-time representation of trades In this framework, observed prices follow as martingale pricing rules, averaging out around a latent fair value, allowing valuation risk to be characterized through aggregation and finite-sample inference.

Crucially, the framework distinguishes pricing risk from model risk. While Bayesian updating governs statistically admissible deviations within the assumed probabilistic structure, persistent tail realizations in prices or arrival dynamics indicate violations of model validity rather than transient noise. In such regimes, continued quoting is no longer an estimation problem but a governance decision. The proposed market withdrawal mechanism enforces this distinction, restricting participation to regimes where the model’s probabilistic foundations remain locally coherent. Structure precedes optimization.
